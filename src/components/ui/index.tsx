...
export function Button({ children, variant = 'secondary', size = 'md', className, onClick, type = 'button', disabled, loading }: ButtonProps) {
  const variants = {
    primary: 'bg-[#D4AF37] text-[#0A0A0A] hover:bg-[#c4a030] active:scale-[0.98]',
    secondary: 'bg-[#1C1C1C] border border-white/10 text-[#F0EDE8] hover:bg-[#252525]',
    ghost: 'text-[#888580] hover:text-[#F0EDE8] hover:bg-white/5',
    danger: 'bg-[#C94F4F]/20 border border-[#C94F4F]/30 text-[#C94F4F] hover:bg-[#C94F4F]/30',
  }
...
export const Input = forwardRef<HTMLInputElement, InputProps>(({ label, error, icon, className, ...props }, ref) => (
  <div className="flex flex-col gap-1.5">
    ...
    <div className="relative">
      ...
      <input ref={ref}
        className={cn('w-full h-10 bg-[#111111] border border-white/10 rounded-xl px-3 text-sm text-[#F0EDE8] placeholder:text-[#555250] focus:outline-none focus:border-[#D4AF37]/50 focus:ring-1 focus:ring-[#D4AF37]/20 transition-colors', icon ? 'pl-9' : '', error ? 'border-[#C94F4F]/50' : '', className ?? '')}
        {...props} />
    </div>
...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(({ label, error, options, className, ...props }, ref) => (
  <div className="flex flex-col gap-1.5">
    {label && <label className="text-xs text-[#888580] uppercase tracking-wide">{label}</label>}
    <select ref={ref}
      className={cn('w-full h-10 bg-[#111111] border border-white/10 rounded-xl px-3 text-sm text-[#F0EDE8] focus:outline-none focus:border-[#D4AF37]/50 focus:ring-1 focus:ring-[#D4AF37]/20 transition-colors', error ? 'border-[#C94F4F]/50' : '', className ?? '')}
      {...props}>
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(({ label, error, className, ...props }, ref) => (
  <div className="flex flex-col gap-1.5">
    {label && <label className="text-xs text-[#888580] uppercase tracking-wide">{label}</label>}
    <textarea ref={ref}
      className={cn('w-full bg-[#111111] border border-white/10 rounded-xl px-3 py-2.5 text-sm text-[#F0EDE8] placeholder:text-[#555250] resize-none focus:outline-none focus:border-[#D4AF37]/50 focus:ring-1 focus:ring-[#D4AF37]/20 transition-colors', error ? 'border-[#C94F4F]/50' : '', className ?? '')}
      {...props} />
    {error && <p className="text-xs text-[#C94F4F]">{error}</p>}
  </div>
))
...
export function Badge({ children, variant = 'muted', className }: BadgeProps) {
  const variants = { gold: 'bg-[#D4AF37]/10 text-[#D4AF37] border-[#D4AF37]/30', success: 'bg-[#3DAA7A]/10 text-[#3DAA7A] border-[#3DAA7A]/30', danger: 'bg-[#C94F4F]/10 text-[#C94F4F] border-[#C94F4F]/30', info: 'bg-[#3A7BD5]/10 text-[#3A7BD5] border-[#3A7BD5]/30', muted: 'bg-white/5 text-[#888580] border-white/10' }
...
export function Modal({ open, onClose, title, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-[#151515] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-white/7">
          ...
          <button onClick={onClose} className="text-[#888580] hover:text-[#F0EDE8] transition-colors">
  ...
  return (
    <Card className="p-5 card-hover">
      <div className="flex items-start justify-between mb-3">
        <p className="text-xs text-[#888580] uppercase tracking-wide">{label}</p>
        {icon && <span className="text-[#888580]">{icon}</span>}
      </div>
      <p className={cn('text-2xl font-medium', accent ? 'text-[#D4AF37]' : 'text-[#F0EDE8]')}>{value}</p>
      {delta && (
        <p className={cn('text-xs mt-2 flex items-center gap-1', deltaUp ? 'text-[#3DAA7A]' : 'text-[#C94F4F]')}>
...
export function EmptyState({ icon, title, description, action }: EmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      <div className="text-4xl mb-4 text-[#555250]">{icon}</div>
      <h3 className="text-base font-medium text-[#F0EDE8] mb-2">{title}</h3>
      <p className="text-sm text-[#888580] mb-4 max-w-xs">{description}</p>
      {action}
    </div>
  )
}
