...
export default function DashboardPage() {
  ...
  // null = still loading initial data to pick the right month
  ...
  // Phase 1: load ALL data first, then pick the best default month
  useEffect(() => {
    if (!user) return
    Promise.all([
      ...
      // Else → use the most recent month that has any data
      // Else → fall back to current month (empty state)
      const cur = currentMonth()
      const curHasData =
        allIncData.some(i => i.date.startsWith(cur)) ||
        allExpData.some(e => e.date.startsWith(cur))

      if (curHasData) {
        setSelectedMonth(cur)
      } else {
        // Find latest month with data across all records
        const allDates = [
          ...allIncData.map(i => i.date.slice(0, 7)),
          ...allExpData.map(e => e.date.slice(0, 7)),
        ]
        if (allDates.length > 0) {
          const latestWithData = allDates.sort((a, b) => b.localeCompare(a))[0]
  ...
  // Phase 2: once we know which month, load month-specific data
  useEffect(() => {
  ...
  // Month options = union of last 6 months + any month that has data, sorted newest first
  const monthsWithData = Array.from(new Set([
    ...last6Months(),
    ...allIncomes.map(i => i.date.slice(0, 7)),
    ...allExpenses.map(e => e.date.slice(0, 7)),
  ])).sort((a, b) => b.localeCompare(a)).slice(0, 24)

  // Still picking the initial month
  if (!selectedMonth) {
    return <AppShell title="Dashboard"><LoadingSkeleton /></AppShell>
  }
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {monthsWithData.map(m => {
              const hasAny =
                allIncomes.some(i => i.date.startsWith(m)) ||
                allExpenses.some(e => e.date.startsWith(m))
              return (
                <option key={m} value={m}>
                  {friendlyMonth(m)}{hasAny ? ' ●' : ''}
                </option>
              )
            })}
          </select>
    ...
    >
      ...
      {!isCurrentMonth && (
        <div className="bg-[#D4AF37]/10 border border-[#D4AF37]/20 rounded-xl px-4 py-2.5 mb-4 flex items-center justify-between">
          <p className="text-xs text-[#D4AF37]">
            📅 Showing data for <strong>{friendlyMonth(selectedMonth)}</strong>
          </p>
          <button
            onClick={() => setSelectedMonth(currentMonth())}
            className="text-xs text-[#D4AF37] hover:underline"
          >
            Switch to {friendlyMonth(currentMonth())}
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <KpiCard
          ...
          delta={!hasData ? 'No transactions this month' : `${savingsRate}% savings rate`}
        ...
        <KpiCard
          ...
          delta={!hasData ? 'No transactions this month' : totalExpenses <= totalIncome ? 'Within income' : 'Over income ⚠'}
          deltaUp={totalExpenses <= totalIncome && hasData}
        />
        <KpiCard
          label="Net Savings"
          value={formatCurrency(Math.abs(monthlySavings))}
          delta={!hasData ? 'No transactions this month' : monthlySavings >= 0 ? `${savingsRate}% of income saved` : 'Spending exceeds income'}
