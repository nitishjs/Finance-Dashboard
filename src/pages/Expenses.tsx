...
export default function ExpensesPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const { error } = editing
      ...
      : await expenseService.create({ ...data, user_id: user.id })
    if (error) { toast({ type: 'error', title: editing ? 'Update failed' : 'Failed to add expense' }); return }
    toast({ type: 'success', title: editing ? 'Expense updated' : 'Expense added' })
  ...
  const handleDelete = async (id: string) => {
    ...
    toast({ type: 'success', title: 'Expense deleted' }); load()
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3 mb-4 md:mb-6">
        <Card className="p-3 md:p-4"><p className="text-[10px] md:text-xs text-[#888580] mb-1">This Month</p><p className="text-lg md:text-xl font-medium text-[#C94F4F]">{formatCurrency(monthlyExp)}</p></Card>
        <Card className="p-3 md:p-4"><p className="text-[10px] md:text-xs text-[#888580] mb-1">All Time</p><p className="text-lg md:text-xl font-medium">{formatCurrency(totalExp)}</p></Card>
        <Card className="p-3 md:p-4"><p className="text-[10px] md:text-xs text-[#888580] mb-1">Entries</p><p className="text-lg md:text-xl font-medium">{expenses.length}</p></Card>
      </div>
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1 -mx-4 px-4 md:mx-0 md:px-0 md:flex-wrap">
        {['all', ...EXPENSE_CATEGORIES].map(cat => (
          <button key={cat} onClick={() => setFilterCat(cat)}
            className={`px-3 py-1.5 rounded-xl text-xs whitespace-nowrap flex-shrink-0 transition-colors ${
              filterCat === cat
                ? 'bg-[#D4AF37]/20 text-[#D4AF37] border border-[#D4AF37]/30'
                : 'bg-[#151515] border border-white/7 text-[#888580] hover:text-[#F0EDE8]'
            }`}>
            {cat === 'all' ? 'All' : cat}
          </button>
        ))}
      ...
      <Card className="p-4 md:p-5">
        {loading ? (
          ...
          <div className="space-y-2">
            {displayed.map(exp => (
              <div key={exp.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-[#1C1C1C] transition-colors group">
                {/* Category dot */}
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 text-base"
                  style={{ background: `${CATEGORY_COLORS[exp.category] ?? '#888'}18` }}>
                  <span style={{ color: CATEGORY_COLORS[exp.category] ?? '#888' }}>↓</span>
                </div>
                {/* Info */}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{exp.name}</p>
                  <p className="text-[10px] text-[#555250]">{exp.category} · {formatDate(exp.date)} · {exp.payment_method}</p>
                </div>
                {/* Amount */}
                <p className="text-sm font-medium text-[#C94F4F] flex-shrink-0">-{formatCurrency(Number(exp.amount))}</p>
                {/* Actions */}
                <div className="flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => handleDelete(exp.id)} className="text-xs text-[#C94F4F] hover:bg-[#C94F4F]/10 px-2 py-1 rounded-lg">✕</button>
                </div>
              </div>
            ))}
          </div>
