    ...
    <AuthLayout title="Welcome back" subtitle="Sign in to your FINGold account">
      <form onSubmit={handleSubmit(onSubmit as any)} className="space-y-4">
        ...
        <div className="flex justify-end">
          <Link to="/forgot-password" className="text-xs text-[#888580] hover:text-[#D4AF37] transition-colors">Forgot password?</Link>
        </div>
      ...
      <div className="hidden md:flex flex-col justify-between w-80 bg-[#111111] border-r border-white/7 p-8 flex-shrink-0">
        <Link to="/" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-[#D4AF37]/15 border border-[#D4AF37]/30 flex items-center justify-center">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="#D4AF37"><path d="M8 1L10.5 5.5H14.5L11.5 8.5L12.5 13L8 10.5L3.5 13L4.5 8.5L1.5 5.5H5.5L8 1Z"/></svg>
          </div>
          <span className="text-sm font-medium">Fin<span className="text-[#D4AF37]">Gold</span></span>
        </Link>
        <div>
          <div className="space-y-4 mb-8">
            {['Track income & expenses', 'Set savings goals', 'Budget by category', 'AI-powered insights'].map(f => (
              <div key={f} className="flex items-center gap-3 text-sm text-[#888580]">
                <span className="text-[#D4AF37] text-base">✦</span>{f}
              </div>
            ))}
          </div>
          <p className="text-xs text-[#555250]">Trusted by 10,000+ users across India</p>
        </div>
      </div>
